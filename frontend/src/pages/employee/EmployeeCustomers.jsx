...
import { Search, Users, Phone, Mail, TrendingUp } from 'lucide-react'
...
export default function EmployeeCustomers() {
  ...
  const fetchCustomers = async () => {
    setLoading(true)
    try {
      // Fetch assigned customers through transfers API
      ...
      transfers.forEach(transfer => {
        const cust = transfer.fromAccount?.customer
        if (cust?.id && !uniqueCustomers[cust.id]) {
          uniqueCustomers[cust.id] = {
            id: cust.id,
  ...
  const handleViewDetails = async (customer) => {
    ...
    try {
      ...
      const filtered = response.data?.filter(acc => acc.customerId === customer.id) || []
  ...
  return (
    <div className="space-y-6">
      {/* Header */}
      <div>
        <h2 className="text-2xl font-bold text-white">My Customers</h2>
        <p className="text-gray-400 mt-1">View and manage your assigned customers</p>
      </div>
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {filteredCustomers.map(customer => (
            <div
              key={customer.id}
              className="p-6 bg-gradient-to-br from-gray-800 to-gray-900 rounded-lg border border-gray-700 hover:border-blue-500/50 transition-all"
            >
              <div className="flex items-center gap-3 mb-4">
                ...
                <div>
                  ...
                  <p className="text-gray-400 text-xs">{customer.recentActivityCount} recent activities</p>
                </div>
              </div>
              ...
              <div className="space-y-2 text-sm text-gray-300 mb-4">
                ...
                <div className="pt-2 border-t border-gray-700 flex items-center gap-2 text-blue-400 font-semibold">
                  <TrendingUp size={14} />
              ...
              <button
                ...
                className="w-full px-4 py-2 bg-blue-500/20 hover:bg-blue-500/30 text-blue-300 border border-blue-500/50 rounded-lg text-sm font-semibold transition-colors"
              >
                View Profile
              </button>
            </div>
              ...
              <div>
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <p className="text-white break-all">{selectedCustomer.email}</p>
                  </div>
                  <div>
                  ...
                  <div>
                    ...
                    <p className="text-blue-400 font-bold">{formatCurrency(selectedCustomer.totalBalance)}</p>
                  </div>
                  <div>
                      ...
                      <div key={account.id} className="p-4 bg-gray-800 rounded-lg border border-gray-700">
                        ...
                        <div className="grid grid-cols-2 gap-4 text-sm text-gray-300">
                          ...
                          <div className="text-right">
                            ...
                            <p className="font-semibold">{formatCurrency(account.availableBalance)}</p>
                          </div>
                        </div>
                      </div>
                    ))}
